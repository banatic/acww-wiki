...
and are therefore uncapped [E: `port/shim/input/touch.c`].
...
3. **The game sees the contact one to two frames late and holds it one frame long**, because
   ...
   is roughly ten frames of `touch = 1` in game RAM, shifted by about two. The port publishes on
   the exact frame. This is a real, measured, unavoidable difference and it is the standing
   candidate explanation for any tap that works on one side only [O: same].
...
- The gate word reading anything with bit 15 set. If touch ever goes dead on either side, check
  that bit before anything else [E: `port/shim/input/touch.c`].
...
One caveat on the instrument, observed and unexplained: `memory.readword` in that Lua build
returned 255 where the ROM writes 0xffff, so it appears to deliver only the low 8 bits. The
tap/no-tap distinction and the coordinates are unaffected -- both are below 256 -- but do not
read those numbers as verified 16-bit values [O: `port/tools/oracle/README.md`; M1].
