...
| D8 | `FX_GetSqrtResult` returns `(SQRT_RESULT + 2^9) >> 10` | GBATEK, same page, "Square Root", with NitroSDK's `FX_SQRT_SHIFT = (32 - FX32_SHIFT) / 2`; the ROM's own byte-identical `src/matched/FX_GetSqrtResult.c` says `10` | **`port/shim/math/divider.c` disagreed; FIXED (IRIS54)** | The shim shifted by **1**, so every `FX_Sqrt` in the port was `2^9 = 512` times too big. `FX_Sqrt(x)` sets `SQRT_PARAM = (u64)x << 32`, so the unit returns `sqrt(X) * 2^22` for a Q12 `x` and the Q12 answer needs `>> 10`. Fixed; pinned by `port/tools/test_math_units.py` |
| D9 | The result of the sqrt unit is UNSIGNED | GBATEK, same: `SQRT_RESULT` is a 32-bit unsigned value, and `CP_GetSqrtResult32` returns `u32` | **`divider.c` disagreed; FIXED (IRIS54)** | The rounding and shift were done on a signed cast, so any root with bit 31 set came back negative -- reachable from `FX_Sqrt(0x7fffffff)` with no 64-bit parameter of the caller's own |
| D10 | The 64-bit remainder is computed without a runtime helper this link lacks | — (a host constraint, not a hardware claim) | **`divider.c` disagreed; FIXED (IRIS54)** | A signed 64-bit `%` compiles to `_allrem`, which `port/platform/lldiv.c` does not supply, so `absdata.py` parked the name in a zeroed arena slot and `FX_DivS32`/`FX_ModS32` were a jump into non-executable memory waiting to happen (`acww.map` carried `__allrem 0x30013000 <absolute>`). The magnitude now goes through the unsigned `%`, which is `_aullrem` |
...
| E5 | HBlank (bit 1) is left clear | GBATEK, same: *"Bit 1: H-Blank flag (Read only) (1=HBlank) (toggled in all lines)"* | **was wrong; FIXED (IRIS54)** | Bit 1 is now set for the duration of the renderer's scanline hook -- the window in which the port stands in for the hardware's HBlank -- and clear outside it, with VCOUNT frozen for the same window so every reader in the handler chain sees the same line. It was not a theoretical gap: ACWW's scene-transition iris writes `WIN0H` from an HBlank callback gated on this bit, so the port painted one `WIN0H` over all 192 scanlines. See `docs/kb/hybrid/render-fixes.md` P17 |
...
**Fix.** Three changes in one expression: `v >= 192 && v <= 261` for bit 0; set bit 2 when `v == (((stat >> 8) & 0xFF) | ((stat >> 7) & 1) << 8)`; and decide HBlank deliberately. **The HBlank clause is now LANDED (IRIS54)**: bit 1 is set for exactly the window in which the port stands in for the hardware's HBlank -- the renderer's scanline hook, which dispatches the ROM's own `OS_IRQTable[1]` -- and clear otherwise; VCOUNT does not advance inside that window. The bit-0 range and the V-counter match were landed earlier.
