...
archives; and 2 streams [H: source account: `docs/kb/port/input-save-audio.md` (archive census); direct ROM-source provenance unresolved]. **Those are three different denominators and "376 SEQ
assets" collapses them** [H: source account: `docs/kb/port/input-save-audio.md` (archive census); M1; direct ROM-source provenance unresolved].
...
`START_PREPARED_SEQ` [H: source account: `docs/kb/port/input-save-audio.md`; direct ROM-source provenance unresolved].
...
and `0x022060a0` [H: source account: `docs/kb/port/input-save-audio.md`; retraction in
...
[H: log/source account: `port/shim/os/pxisend.c`; `docs/kb/hybrid/hardware-services.md` section 1, tag 7; receipt provenance unresolved]. That
...
`docs/log/cycle40-keyboard-gate-probe.md` `off-D44`; `scratchpad/cycle40/runs/off-D44`]. The archive comes from the filesystem
...
both screens black [H: log/source account: `docs/log/cycle40-keyboard-gate-probe.md` CARD40..SND40; receipt provenance unresolved]. Fourteen sound
...
everything else [H: log/source account: `port/shim/os/pxisend.c`, `port/shim/audio/w12_sndservice.h`;
...
[H: log/source account: `docs/kb/hybrid/audio.md` sections 1, 6 and 7; receipt provenance unresolved]. **There is no host archive**: the ROM's own
...
[H: log/source account: `docs/kb/hybrid/audio-banks.md`, section 2; receipt provenance unresolved]. The first silent run was `swav.c` reading `waveOffset[]` as file offsets
...
[H: log/source account: `docs/kb/hybrid/audio.md` section 3; receipt provenance unresolved]. The census is uncapped now and runs whether or not the
...
changes the game rather than the sound [H: log/source account: `docs/kb/hybrid/audio.md` section 3; receipt provenance unresolved]. **This ROM never sends id 33**
...
`unk_autoload_2.bin+0x54dcc` [H: source account: those images; `port/tools/test_sndrender.py` re-reads all three
at pinned offsets on every run; direct ROM-source provenance unresolved]. Two of the three grade-H reconstructions were byte-identical;
...
table against -60 in the plain one) [H: source account: the ROM images; P: NitroSDK `snd_seq.c`,
`snd_exchannel.c`; direct ROM-source provenance unresolved]. **Entry 0 of both tables is -723 in this ROM where the public NitroSDK has
...
where this ROM does not (STYLE rule 7) [H: source account: the images vs P: the public source; direct ROM-source provenance unresolved]. The allocator
...
doing that all along and nothing showed it [H: log/source account: `docs/kb/hybrid/audio.md` section 8(c); receipt provenance unresolved].
...
audit's build order are done [H: source account: `../audits/audio-design.md` section 2, "Build order"; direct ROM-source provenance unresolved].
...
archive 4, wave 42, one 6,020-byte DS ADPCM payload [H: source account: `docs/kb/port/input-save-audio.md`;
confirmed field by field in `../audits/audio-design.md` section 3; direct ROM-source provenance unresolved]. The two offline decoders
...
[H: log/source account: `docs/log/cycle40-keyboard-gate-probe.md` AUDIO6 and AUDIO7; receipt provenance unresolved]. On the OFF recipe the port
...
| `NNS_SndArcGetSeqArcParam` | autoload_2 (NNS) | the INFO accessor whose NULL sent the ROM to its fatal path in the port | [S: `src/matched/NNS_SndArcGetSeqArcParam.c`] [H: log/source account: `docs/log/cycle40-keyboard-gate-probe.md` SND40; receipt provenance unresolved] |
...
| `PXI_SendWordByFifo` | autoload_2 | the FIFO word send tag 7 rides on | [S: `src/matched/PXI_SendWordByFifo.c`] [H: log/source account: replaced by `port/shim/os/pxisend.c`; receipt provenance unresolved] |
...
| `func_020eedb8`, `func_020f609c`, `func_020f6350`, `func_020f7bc4` | main | the four game-glue callers of the start boundary; not decompiled | [H: source account: `docs/kb/port/input-save-audio.md`; direct ROM-source provenance unresolved] |
| `acww_snd_command` (`driver.c`) | port | the host ARM7: sixteen players, thirty-two tracks, a sixteen-channel mixer, the census | [H: log/source account: `port/shim/audio/driver.c`; `docs/kb/hybrid/audio.md` section 3; receipt provenance unresolved] |
| `capture.c` | port | the two capture units, `OUTPUT_SELECTOR`, the surround decay, the hardware channels | [H: log/source account: `port/shim/audio/capture.c`; `docs/kb/hybrid/audio.md` section 7; receipt provenance unresolved] |
...
| `SND_CalcDecibelSquare` `arm7.bin+0xf1cc` (table) | ARM7 image | the table velocity, both track volumes, the player volume and the sustain level go through | [H: source account: `extract/adm-kr/arm7/arm7.bin`, re-read by `port/tools/test_sndrender.py`; direct ROM-source provenance unresolved] |
| `SNDi_DecibelTable` `unk_autoload_2.bin+0x54dcc`, `AttackCoeffTable` `arm7.bin+0xf2e0` | images | the other two constant tables; entry 0 of both decibel tables is **-723** in this ROM | [H: source account: the `extract/adm-kr/arm7/arm7.bin` and `extract/adm-kr/arm9/unk_autoload_2.bin` two images; direct ROM-source provenance unresolved] |

## Data it reads and writes

| address or field | meaning | who writes | who reads |
|---|---|---|---|
| `0x02204620` | the shared work block; its first word is `finishCommandTag` | the ARM7 (the port: `snd_arm7`) | `SND_RecvCommandReply`, `SND_IsFinishedCommandTag` [H: source account: `docs/kb/port/input-save-audio.md`; direct ROM-source provenance unresolved] |
| `0x022060a0` | the pointer to that block | `SNDi_InitSharedWork` | the driver [H: source account: `docs/kb/port/input-save-audio.md`; direct ROM-source provenance unresolved] |
| `0x022048a0` | the 256-entry command array | `SND_AllocCommand` | the ARM7 [H: source account: `docs/kb/port/input-save-audio.md`; direct ROM-source provenance unresolved] |
| `0x022045c8` / `0x022045d8` | command free-list head and tail | `SND_AllocCommand`, `SND_RecvCommandReply` | the allocator [H: source account: `docs/kb/port/input-save-audio.md`; direct ROM-source provenance unresolved] |
| `0x022045d0` / `0x022045d4` | reserve-list head and end | `SND_PushCommand` | `SND_FlushCommand` [H: source account: `docs/kb/port/input-save-audio.md`; direct ROM-source provenance unresolved] |
| `0x022045dc` / `0x022045e0` / `0x022045e4` | pending read index, write index, batch count | `SND_FlushCommand` | the reply path [H: source account: `docs/kb/port/input-save-audio.md`; direct ROM-source provenance unresolved] |
| `0x022045cc` / `0x022045e8` | reclaimed tag and current submission tag | the reply path / `SND_FlushCommand` | `SND_WaitForCommandProc` [H: source account: `docs/kb/port/input-save-audio.md`; direct ROM-source provenance unresolved] |
...
| `sound_data.sdat` | the whole archive: 336 sequences, 216 sequence archives, 943 banks, 14 wave archives, 2 streams | the ROM image | `NNS_SndArc*` through the filesystem [H: source account: `docs/kb/port/input-save-audio.md`; direct ROM-source provenance unresolved] |
...
[H: log/source account: `docs/kb/hybrid/audio.md` section 6; receipt provenance unresolved]. Without a ROM at all,
...
the fixtures [H: log/source account: `docs/kb/hybrid/audio.md`, section 8(d); receipt provenance unresolved].
...
- ~~The silent ARM7 is adequate for the game's logic indefinitely.~~ **Settled 2026-09-09
  ...
  wrong was "adequate" [H: log/source account: `docs/kb/hybrid/audio.md` section 8(c);
...
- **H: the mix is CORRECT, not merely present.** Nothing has yet compared the port's driver
  ...
  finding out why [H: log/source account: `docs/kb/hybrid/audio-differential.md` section A10; receipt provenance unresolved].
...
- ACWW selects background music by the hour, as the series does. **No such table or function was
  ...
  [H: source account: absence in `src/matched`; direct ROM-source provenance unresolved]. If it exists it is inside one of the four undecompiled callers
...
- ~~Archive 0 index 121 is reachable in ordinary play and would make the first sound.~~ **The
  ...
  [H: log/source account: `docs/kb/hybrid/audio.md` section 8(a); receipt provenance unresolved]. Whether index 121 is reached at all is still
...
- Archive 0 index 121 is reachable in ordinary play and would make the first sound. It was not
  ...
  [H: source account: `docs/kb/port/input-save-audio.md`; M11; direct ROM-source provenance unresolved]. Settled by an uncapped request trace over a town
  run.
