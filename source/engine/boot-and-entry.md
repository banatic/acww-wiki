...
flushes the cache [S: `Entry`, main, `port/platform/nitromain.c` header]. None of that is a
decompilable C body — it is one of the assembly translation units the host build cannot
compile at all [S: `port/platform/nitromain.c` header; `port/tools/expected_failed_tus.txt`].
...
[S: `Entry`, main, quoted in `port/platform/win32.c:2340-2348`].
...
[S: `port/platform/nitromain.c` header]. The symbol table cuts a 0x174-byte Thumb symbol
...
[S: `func_02000b6c`, main, `port/platform/nitromain.c` header]. This is the first of several
...
[S: `port/platform/nitromain.c:33-46, 62-66`]. It then reads `REG_IME` (`0x04000208`) and
writes 1 into it, calls `OS_EnableInterrupts`, and calls `func_0206e518`
[S: `port/platform/nitromain.c:68-75`]. The `REG_IME` read is dead as a value and live as a
hardware access — the ROM overwrites the loaded register immediately — which is why the port
keeps it as a volatile read [S: `port/platform/nitromain.c:67-71`].
...
[S: `port/platform/nitromain.c:77-82`]. Both paths end the same way, with `func_020b1b84`
and then `func_0206e560`, and the routine never returns
[S: `port/platform/nitromain.c:84-85`]. `func_0206e560` is where the game proper begins;
...
own crash screen, with both screens black [E: `docs/log/cycle40-keyboard-gate-probe.md`,
CARD40..SND40; observed from frame ~830 of an interpreter run].
...
[S: `func_020b1a74`, main, `port/interp/interp_boot.c:29`]. Its LAST call is `func_020ea48c`,
the game's main heap carve-out, so nothing after the bring-up can run until that heap exists
[S: `func_020ea48c`, autoload_2, `port/shim/boot/heapinit.c` header]. That function reads
...
from arena 0 [S: `func_020ea48c`, autoload_2, `port/shim/boot/heapinit.c` header]. See
...
[E: `docs/log/cycle40-keyboard-gate-probe.md` ENTRY40, runs `off-D17`..`off-D20`].
...
the interpreter's hooks installed first [S: `port/interp/interp_boot.c:28-30, 404-418`]. The
native bring-up, the native `__init_cpp` and the native sound boot are all skipped, and the
port says so on stdout [S: `port/platform/win32.c:2355-2405`].
...
[S: `port/interp/interp_boot.c:395-401`]. The interpreted system stack top is `0x027e3e00`,
placed below NitroSDK's IRQ (0x100) and SVC (0x40) stacks at the top of DTCM
[S: `port/interp/interp_boot.c:11-14, 31`].
...
`off-D16`]. And the sound command layer had to move AFTER the interpreted bring-up, because
`SND_CommandInit` waits for the PXI bit that the ROM's own `PXI_Init` sets
[E: `docs/log/cycle40-keyboard-gate-probe.md` ENTRY40, `off-D17`..`off-D20`].
...
supplies it by address because there is no name to register [S: `port/tools/interp_registry.py:77-80`;
E: `docs/log/cycle40-keyboard-gate-probe.md` ENTRY40, `off-D18`, where the read spun forever].

### After boot: what a frame is

The NDS has no frame loop [S: `port/shim/os/vblank.c:11-14`]. The game's loop ends by blocking in `VBlankIntrWait`
(`0x020002c7`, three instructions around the BIOS `IntrWait`), and the vertical-blank
interrupt is what makes time pass [S: `VBlankIntrWait`, main, `port/shim/os/vblank.c` header].
The idle thread's whole body is `while (1) OS_Halt();`, and `OS_Halt` is the CP15
wait-for-interrupt [S: `OSi_IdleThreadProc` / `OS_Halt`, itcm, `port/shim/os/halt.c` header].
...
the counter wraps to line 0 [S: `port/interp/interp_boot.c:239-254`;
E: `docs/log/cycle40-keyboard-gate-probe.md` CARD40..SND40, `off-D36`]. Without that rule
nothing on the state-machine path ever reached a frame boundary at all
[E: same run]. See `threads-and-interrupts.md`.
...
taps do not [E+O: same comparison]. **That divergence is settled (ORACLE42)**: the tap at 24,600
...
|---|---|---|---|
| `Entry` (`0x02000800`) | main | crt0: CP15, stacks, OAM/palette clear, cache flush | [S: `port/platform/nitromain.c` header] |
| `func_020b1a74` (Thumb) | main | hardware, wireless and filesystem bring-up | [S: `port/interp/interp_boot.c:29`] |
| `func_020ea48c` | autoload_2 | the bring-up's last call: carve the game heap from arena 0 | [S: `port/shim/boot/heapinit.c` header] |
| `func_02138968` (ARM) | main | `__init_cpp`, the C++ static initialisers | [S: `port/interp/interp_boot.c:30`] |
| `0x02000c38` (Thumb, 0x60 bytes) | main | the real `NitroMain`; no symbol starts here | [S: `port/platform/nitromain.c` header] |
| `OS_InitTick` / `OS_InitAlarm` / `OS_InitThread` | autoload_2 | OS services, in that order | [S: `port/platform/nitromain.c:33-35`] |
| `OS_SetUserExceptionHandler` (`0x0211628c`) | autoload_2 | installs `func_0206e6c4` with arg `0x0220433c` | [S: `port/platform/nitromain.c:36, 46`] |
| `OS_EnableInterrupts` (`0x01ffa314`) | itcm | interrupts on | [S: `port/platform/nitromain.c:37`] |
| `func_0206e518`, `func_020b1b84`, `func_0206e560` | main | the hand-over to the game; never returns | [S: `port/platform/nitromain.c:75, 84-85`] |
| `func_0206e6c4` / `func_0206e3f4` | main | fatal path: stores the register table, then the crash screen | [E: cycle40 log, CARD40..SND40] |
| `VBlankIntrWait` (`0x020002c7`) | main | the frame boundary the game blocks on | [S: `port/shim/os/vblank.c` header] |
| `OS_Halt` | itcm | the idle thread's body | [S: `port/shim/os/halt.c` header] |
...
|---|---|---|---|
| `0x04000208` (`REG_IME`) | interrupt master enable; set to 1 twice on the flagged path | `NitroMain` | the interrupt controller [S: `port/platform/nitromain.c:49, 68-81`] |
| `0x027ffc20` | boot flag; 1 selects the second boot path | set before `NitroMain` | `NitroMain` [S: `port/platform/nitromain.c:50, 77`] |
| `0x0220433c` | the user exception handler's argument word | ROM literal pool | `OS_SetUserExceptionHandler` [S: `port/platform/nitromain.c:48`] |
| `0x0213fde0` | the exception register table on a fatal fault | `func_0206e6c4` | the crash screen, and `ACWW_INTERP_PEEK` [E: cycle40 log, CARD40..SND40] |
| `0x027fff9c` | the interrupt-vector slot crt0 writes | `Entry` | the vector [S: `port/platform/win32.c:78-83`] |
| `0x027e3e00` | interpreted system stack top (port only) | `interp_boot.c` | the interpreter [S: `port/interp/interp_boot.c:31`] |
...
oracle can pin the same instant [S: `docs/kb/hybrid/recipes.md` sections 3 and 6].
