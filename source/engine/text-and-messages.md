...
module; overlays only *request* messages [S: `port/TAXI-ROAD.md`, "The message system is not in
an overlay"]. The window occupies two runs in `main`, `0x02066bd0`-`0x02068e00` and
`0x020a8100`-`0x020a9d60` [S: `port/TAXI-ROAD.md`, subsystem table].
...
`ge_`, `fu_`, `ko_`, `ha_` [S: `func_0200366c`, `main`, `src/matched/func_0200366c.c`;
...
[S: `func_02067344`, `main`, `src/matched/func_02067344.c`]. `func_0206726c` opens it through
the BMG reader held at `self+0xb14` and hands the resulting text to the string object at
`self+0x14ac` [S: `func_0206726c`, `main`, `src/matched/func_0206726c.c`]. The window is then
driven by a six-state pointer-to-member table; state 2's update is where a button press advances
a line [S: `func_02066dc4`, `main`, `src/matched/func_02066dc4.c`].
...
choice lists [S: `extract/adm-kr/arm9/arm9.bin`, path-format string literals]. The `%s` that
...
[S: `extract/adm-kr/files/script/KOR/`, recursive census]. `message/` is 1,526 of them, split
into nine personality directories: `bo`, `ta`, `ge`, `fu`, `ha`, `ko`, plus `sp`, `obj` and
`Other` [S: `extract/adm-kr/files/script/KOR/message/`, subdirectory listing]. `bo` and `ta`
...
template [S: `extract/adm-kr/files/script/KOR/message/bo/`,
`extract/adm-kr/files/script/KOR/message/ta/`, subdirectory listings and counts]. The remaining
groups are `mail/` (168 files in `msg`, `ps`, `super`), `mailz/` (48 in `msga`, `msgb`, `psz`,
`superz`), `string/` (36 loose files), `bbs/` (10), `select/` (2) and `2d/` (1)
[S: `extract/adm-kr/files/script/KOR/`, per-directory census]. `string/` is where the item and
...
[S: `extract/adm-kr/files/`, first-four-bytes census]. Its header is `MESGbmg1`, a file-size
field that matches exactly, a block count of 2, and an encoding byte of 2 — the BMG code for
UTF-16 [S: `extract/adm-kr/files/script/KOR/message/Other/test_.bmg`, bytes 0..17]. The two
blocks are `INF1` (16 entries of 12 bytes) and `DAT1` (800 bytes), and `DAT1`'s payload is
UTF-16LE text after an initial null entry [S: same file, `INF1` +8, `DAT1` payload]. Every
...
`port/TAXI-ROAD.md`, "This ROM names things by asset path"]. Every game-side text function is a
`func_02xxxxxx`.
...
gap between two named symbols [S: `config/adm-kr/arm9/symbols.txt`, both rows
...
`font/` [S: `extract/adm-kr/files/font/`, 12 files]. Each `head` is exactly 8 bytes and reads as
...
[S: `extract/adm-kr/files/font/font{A,B,C,D}_head.bin`, all 8 bytes]. The `img` sizes confirm a
...
[S: `func_020516b8`, `main`, `src/matched/func_020516b8.c`], `func_0205171c` fetches one glyph's
width record [S: `func_0205171c`, `main`, `src/matched/func_0205171c.c`], and `func_020512d4`
and `func_0205156c` wrap the lookup with the `0xFF20` fallback
[S: `src/matched/func_020512d4.c`, `src/matched/func_0205156c.c`]. `func_020a8e8c` dispatches
between the two wrappers on a flag at `self+0x7c` [S: `func_020a8e8c`, `main`,
`src/matched/func_020a8e8c.c`]. fontA's attribute table is LZ77-compressed and holds 3,314
entries of `{u16 code, u8 width, u8}` [S: `port/shim/game/glyphcode.c`, header].
...
`/a_mes/...` path literals]. The title-plate members are requested by `ov147`
...
between them [S: `port/TAXI-ROAD.md`, keyboard trio row]. The Hangul input method itself is the
...
[S: `func_020efc74`, `autoload_2`, `src/matched/func_020efc74.c`, header]. The keyboard artwork
...
`extract/adm-kr/arm9_overlays/ov124.bin` and `ov095.bin`, path literals]. Like everything else
...
its assets [S: `port/TAXI-ROAD.md`, "This ROM names things by asset path"].
...
[S: `port/shim/ui/w40_ov126_022a1228_gateprobe.c`, header]. Its gate reads the current touch
...
`0x022a084c`; `port/shim/ui/w90_ov126_022a07e8_touchwords.c`, header]. The hit test is
...
|---|---|---|---|
| `0x020d72a4` | six-pointer table of personality prefixes | ROM data | `func_0200366c` [S: `port/TAXI-ROAD.md`] |
...
| BMG `INF1` entry, 12 bytes | one message record | the packer | the BMG reader [S: `.../Other/test_.bmg`] |
...
[E: `port/VISIBLE-STATE.md`, the glyph observation]. The cause was `func_020a8e8c` dropping the
...
- **The `0xf7` compression type is specific to `.bmg`.** All 1,790 compressed message files carry
  it and no other file does [S: `extract/adm-kr/files/`, byte-4 histogram]. Whether it is a
...
- **The string walk is big-endian while the BMG payload is UTF-16LE.** `func_020a9368` composes
  `(p[1] << 8) | p[0]` [S: `port/shim/game/glyphcode.c`], but the `DAT1` payload of `test_.bmg`
  reads as little-endian [S: same file]. Either the reader byte-swaps on load or one of the two
...
copied [S: `func_02064658` disassembly; port/shim/game/lzread.c]. On the interpreter path
that ROM code runs and calls the registered host `MI_UncompressLZ8` once per chunk, which
is why the town dialogue renders correctly [E: `tap-D63` 37,500]. Fixture:
