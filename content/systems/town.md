...
순방향 레시피 아래의 확인된 마을 `0x8365`에서는 프레임 48,000에 맵 워드 2,057개 중 차이가 1개이다; 에이커 바이트 36개와 건물 셀 열일곱 개 모두 일치하며, 마을 회관은 두 생산자 모두에서 (72, 36)에 있다 [E: `scratchpad/oracle49/RECEIPTS.md`; log:
`docs/log/cycle41-gameplay.md` O49-4].
다른 워드는 생성된 건물이나 에이커의 차이가 아니라, 생성 뒤 실제 플레이 만 이천 프레임 후에 기록된 느슨한 아이템 `0x1555`이다
[E: `scratchpad/oracle49/RECEIPTS.md`; log: `docs/log/cycle41-gameplay.md` O49-4].

**역방향 레시피 아래에서 id를 공유한다고 배치를 공유하는 것은 아니다:** EXIT48은 마을 회관 도어매트를 포트에서는 (40,38)에, 무장된 역방향 원본에서는 (40,22)에서 발견했고, 각 생산자는 프레임 38,838에 자기 진입 위치를 기록해 퇴장 시 재생했다
[E: `scratchpad/exit48/RECEIPTS.md`; log: `docs/log/cycle41-gameplay.md` X48-2, X48-3, X48-4].
따라서 그 열여섯 타일은 한 배치 안에서 잘못 놓인 플레이어가 아니라 두 개의 마을 배치를 기술한다; ORACLE49는 역방향 레시피의 불일치를 배치 버스트에서 찾아냈는데, 무장된 원본은 인덱스 3,780에서, 포트는 3,782에서 진입한다 [E: `scratchpad/exit48/RECEIPTS.md`,
...
**맵이 세이브 이미지 어디에 있는지, 그리고 타일이 주소로 어떻게 대응되는지.** 세이브 `+0xd304`(`0x021e9aac`)의 에이커 맵 객체는 **원시 에이커 바이트 36개**, 6x6이고, 아이템 레이어(ITEM LAYER)가 그 뒤 세이브 `+0xd328`(`0x021e9ad0`)에 이어진다. **아이템 레이어는 안쪽(INNER) 4x4 에이커만 덮는다** -- 16x16 하프워드로 된 4x4 에이커는 정확히 4,096하프워드 / 8,192바이트이므로, 두 축 모두 타일 16..79를 담고 테두리 에이커는 아이템 블록을 전혀 갖지 않는다:

    idx  = (addr - 0x021e9ad0) / 2
    tile = (16 + 16*((idx//256) % 4) + (idx%256) % 16,
            16 + 16*((idx//256) // 4) + (idx%256) // 16)

측정됨(MEASURED): 그 산술을 포트의 대조 마을 `0xc66e`에 적용하면, 실내(INDOORS)에서 찍은 스냅샷으로부터 실제 필드 없이 아래 문 표의 건물 행 아홉 개 -- 마을 회관 (40, 36), 박물관 (71, 26), 너굴 상점 (22, 52) 등 -- 를 하나도 빠짐없이 재현한다
...
| 종류 | 무엇인가 | 걸을 수 있는가? |
|---|---|---|
| `0x03` `0x04` `0x09` `0x17` `0x1d` `0x1e` | 풀, 집터, 두 번째 풀 색조, 이음새 | 예 |
| `0x1f`..`0x57` | 경사, 비탈, 다리 상판, 해변 | 예 |
| `0x08` | 바다 | 아니오 |
| `0x0a` | 건물의 발자국 | 아니오 |
| `0x0b`..`0x14` | 강 | 아니오 |
| `0x15` | 마을 바깥(테두리 에이커는 전부 `0x15`) | 아니오 |

레퍼런스 마을에서 이는 **보행 가능 타일 3,256개 중 3,214개가 한 컴포넌트**를 이루며, 그 안에 플레이어와 모든 문이 들어 있다; 나머지 두 컴포넌트는 41타일과 1타일이다
[E: docs/log/cycle41-gameplay.md NAV42, `scratchpad/nav42/RECEIPTS.md`]. 아이템(ITEM) 레이어는 종류 위에 추가로 막는다: 건물 id `0x5000..0x5021`, 그리고 ROM 자체의 9항 술어가 골라내는 나무/바위 계열 [S: src/matched/func_ov003_0220cd84.c]. **그 술어는 종류 바이트가 아니라 아이템(ITEM) 워드를 검사한다** -- 마을의 아이템 레이어는 `0x002a`를 85번, `0x0053`을 32번, `0x0068`을 15번, `0x0061`을 10번 갖고 있으며 모두 그 대역 안에 있다 -- 이로써 그것이 물/절벽/길/풀을 분할한다는 이 페이지의 가설은 폐기된다.

**아이템 레이어는 흔든 나무와 떨어진 과일이 기록되는 곳이기도 하므로**, 지형만이 아니라 실제 게임플레이 필드이다. 한 마을의 오렌지 나무를 한 번 흔드는 동안 측정한, 움직인 워드 넷 [E: docs/log/cycle42-save.md `## PICKUP43`,
`scratchpad/pickup43/RECEIPTS.md`]:

| id | 무엇인가 | 등급 |
|---|---|---|
| `0x0043` | **열매가 달린** 오렌지 나무(그 마을에 28그루) | E: 흔들기 전 흔든 타일의 워드 |
| `0x0044` | 같은 나무가 **흔들려 빈 상태** | E: 흔든 뒤 같은 워드 |
...
|---|---|---|---|---|---|
| 마을 회관 | `0x5000` | (40, 36) | (40, 38) | 북 | E: map panel (150,172), NAV42 |
| 주민 집 1-3 | `0x5001`-`0x5003` | (51,60) (61,69) (76,34) | (51,62) (61,71) (76,36) | 북 | S: this page, `0x5001`..`0x5008`; E: the three blue map icons |
| 빈 집터 | `0x500a` | 9개 | -- | -- | 타일 하나, 발자국 없음, 문 없음 |
| 문 | `0x500b` | (39, 16) | (40, 16) | 북 | E: the red arch at the top of the map |
| 에이블 시스터즈 | `0x500c` | (27, 54) | (25, 56) | 북 | E: the second red building on the west side |
| 너굴 상점 | `0x500d` | (22, 52) | (20, 54) | 북 | S: this page, the shop is `0x500d`; the notch is at the block's bottom-LEFT corner |
| 박물관 | `0x5011` | (71, 26) | (71, 27) | 북 | E: the white columned icon |
| 플레이어의 집 | `0x5014` | (41, 53) | (41, 55) | 북 | E: the GREEN legend icon; `gp-D9` entered this building |
| 표지판 | `0x501c` | (35, 38) | (35, 39) | 북 | E: 2x2, beside the town hall |
...
측정됨(GAMEPLAY44), 건물이 아닌 열한 번째 행: 주민 액터(ACTORS). `0x021d1d4c`의 매니저가 실제 NPC들을 담고 있고 `--list`는 이들을 월드 위치와 각각에게 말을 걸 타일과 함께 출력한다; `goto.py --to actor:<n>`은 하나에게 걸어가 A를 누른다. `wiki/systems/villagers.md`를 보라.
...
| `0x021e9aac`(세이브 `+0xd304`) | 에이커 맵 객체 -- 원시 에이커 바이트 36개, 6x6 | `func_0209ebec`; 저장은 `func_0204e728` 아래의 pc `0x0204ea54` | 필드/렌더 |
| `0x021e9ad0`(세이브 `+0xd328`) | 아이템 레이어(ITEM LAYER), 8,192바이트 = 안쪽 4x4 에이커, 타일 16..79 | 생성 버스트 | 필드/렌더, `savemap.py` |
