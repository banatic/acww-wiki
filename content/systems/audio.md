...
아카이브, 2개의 스트림이 있다 [S: same]. **이는 세 개의 서로 다른 분모이며 "376개의 SEQ
에셋"이라는 표현은 이를 뭉뚱그린다** [S: same; M1].
...
[E: `port/shim/os/pxisend.c`; `docs/kb/hybrid/hardware-services.md` section 1, tag 7]. 그
...
[E: `docs/kb/hybrid/audio.md` sections 1, 6 and 7]. **호스트 아카이브는 없다**: ROM 자체의
`NNS_SndArcInit`이 포트의 가상 카트리지를 통해 `/sound_data.sdat`를 열고, `PREPARE_SEQ`가 실어
나르는 포인터는 NDS 주소이므로 `sdat.c`는 링크에서 빠진 채로 남는다
[E: same, section 2]. 첫 무음 실행은 `swav.c`가 `waveOffset[]`를 파일 오프셋으로 읽은 것이
...
인공물이다(M1) [E: `docs/kb/hybrid/audio.md` section 3]. 센서스는 이제 상한이 없고 드라이버가
...
아니라 게임을 바꾸기 때문이다 [E: same]. **이 ROM은 id 33 `READ_DRIVER_INFO`를 결코 보내지
...
`unk_autoload_2.bin+0x54dcc`에 있다 [S: those images; `port/tools/test_sndrender.py` re-reads all three
at pinned offsets on every run]. 등급 H 재구성 셋 중 둘은 바이트 단위로 동일했다. **오류는
...
[S: the ROM images; P: NitroSDK `snd_seq.c`,
`snd_exchannel.c`]. **두 테이블의 항목 0은 이 ROM에서 -723이며, 공개 NitroSDK에서는
-32768이다**. 이는 라이브러리의 다른 리비전이며, 그 불일치는 잡음이 아니라 내용이다. 공개 값으로
만든 드라이버는 0 항을 포함한 체인을 무음으로 클램프하지만 이 ROM은 그러지 않는다(STYLE 규칙 7)
[S: the images vs P: the public source]. 할당자는 라이브러리 고유의 순서
...
이를 드러내지 않았다 [E: `docs/kb/hybrid/audio.md` section 8(c)].
...
감사의 빌드 순서 여덟 단계가 모두 완료되었다 [S: `../audits/audio-design.md` section 2, "Build order"].
...
6,020바이트의 DS ADPCM 페이로드 하나다 [S: `docs/kb/port/input-save-audio.md`;
confirmed field by field in `../audits/audio-design.md` section 3]. 12,033샘플 대 12,032샘플로
...
[E: `docs/log/cycle40-keyboard-gate-probe.md` AUDIO6 and AUDIO7]. OFF 레시피에서 포트는 시퀀스
...
| `acww_snd_command` (`driver.c`) | 포트 | 호스트 ARM7: 플레이어 열여섯 개, 트랙 서른두 개, 16채널 믹서, 센서스 | [E: `port/shim/audio/driver.c`; `docs/kb/hybrid/audio.md` section 3] |
| `capture.c` | 포트 | 캡처 유닛 두 개, `OUTPUT_SELECTOR`, 서라운드 감쇠, 하드웨어 채널 | [E: `port/shim/audio/capture.c`; `docs/kb/hybrid/audio.md` section 7] |
...
| `SND_CalcDecibelSquare` `arm7.bin+0xf1cc` (테이블) | ARM7 이미지 | 벨로시티, 두 트랙 볼륨, 플레이어 볼륨, 서스테인 레벨이 거치는 테이블 | [S: `extract/adm-kr/arm7/arm7.bin`, re-read by `port/tools/test_sndrender.py`] |
| `SNDi_DecibelTable` `unk_autoload_2.bin+0x54dcc`, `AttackCoeffTable` `arm7.bin+0xf2e0` | 이미지 | 나머지 상수 테이블 두 개; 두 데시벨 테이블의 항목 0은 이 ROM에서 **-723**이다 | [S: the same two images] |
...
- ~~침묵하는 ARM7은 게임의 로직에 무기한 충분하다.~~ **2026-09-09 해결됨(AUDIO6), 그리고
  ...
  [E: `docs/kb/hybrid/audio.md` section 8(c);
...
- **H: 믹스는 단지 존재하는 것이 아니라 정확하다.** 아직 아무것도 포트의 드라이버 상태를 원본의
  ...
  **마일스톤 8이 작성된 세션에서 에뮬레이터가 시작되지 않았다**. 세 번 시도했고, 매번 프로세스는
  ...
  100`을 그대로 실행하고 이유를 알아내면 해결된다 [E: `docs/kb/hybrid/audio.md` section 10].
...
- ~~아카이브 0 인덱스 121은 일반 플레이에서 도달 가능하며 첫 소리를 낼 것이다.~~ **후반부는
  ...
  [E: `docs/kb/hybrid/audio.md` section 8(a)]. 인덱스 121에 도달하기는 하는지는 여전히 열려
...
- 아카이브 0 인덱스 121은 일반 플레이에서 도달 가능하며 첫 소리를 낼 것이다. 측정된 콜드 START
  ...
  [S: `docs/kb/port/input-save-audio.md`; M11]. 마을 실행에 대한 상한 없는 요청 트레이스로
  해결된다.
