...
캐시를 플러시한다 [S: `Entry`, main, `port/platform/nitromain.c` header]. 이 중 어느 것도
디컴파일 가능한 C 본체가 아니다 — 호스트 빌드가 전혀 컴파일할 수 없는 어셈블리 번역 단위 중
하나이다 [S: `port/platform/nitromain.c` header; `port/tools/expected_failed_tus.txt`].
...
[S: `port/platform/nitromain.c` header]. 심볼 테이블은 0xcc바이트 앞에서 시작하는 0x174바이트 Thumb 심볼
`func_02000b6c`를 잘라 두었으므로, 진입 루틴은 0xcc바이트의
읽기 전용 버전 문자열이 앞에 붙은 채로 도착한다 — 정확히 `0x02000b6c + 0xcc = 0x02000c38`이다
[S: `func_02000b6c`, main, `port/platform/nitromain.c` header]. 이것은 ROM이 아니라
...
[S: `port/platform/nitromain.c:33-46, 62-66`]. 그런 다음 `REG_IME`(`0x04000208`)를 읽고
1을 쓰고, `OS_EnableInterrupts`를 호출하고, `func_0206e518`을 호출한다
[S: `port/platform/nitromain.c:68-75`]. `REG_IME` 읽기는 값으로서는 죽어 있지만
하드웨어 접근으로서는 살아 있다 — ROM은 로드된 레지스터를 즉시 덮어쓴다 — 그래서 포트는
이를 volatile 읽기로 유지한다 [S: `port/platform/nitromain.c:67-71`].

`0x027ffc20`의 부팅 플래그 뒤에는 두 번째 경로가 있다: 이 값이 1이면 루틴은
`func_0206e464`도 호출하고, `REG_IME`를 다시 건드리고, `func_02116a1c(2)`를 호출한다
[S: `port/platform/nitromain.c:77-82`]. 두 경로 모두 같은 방식으로, `func_020b1b84`
다음 `func_0206e560`으로 끝나며, 루틴은 절대 반환하지 않는다
[S: `port/platform/nitromain.c:84-85`]. `func_0206e560`이 게임 본체가 시작되는 곳이다;
...
ROM 자체의 크래시 화면이다 [E: `docs/log/cycle40-keyboard-gate-probe.md`,
CARD40..SND40; observed from frame ~830 of an interpreter run].

### 초기 기동 루틴과 그 마지막 호출

`func_020b1a74`는 Thumb이며 하드웨어, 무선, 파일시스템 초기 기동을 수행한다
[S: `func_020b1a74`, main, `port/interp/interp_boot.c:29`]. 그 마지막 호출은 게임의
메인 힙 분할인 `func_020ea48c`이므로, 그 힙이 존재하기 전까지는 초기 기동 이후의 어떤 것도 실행될 수 없다
[S: `func_020ea48c`, autoload_2, `port/shim/boot/heapinit.c` header]. 이 함수는
`OS_GetArenaLo(0)`과 `OS_GetArenaHi(0)`을 읽고, 빼기 전에 하위 끝을 32바이트로
올림하여 크기가 이미 정렬 패딩을 감당하도록 한 뒤, 나머지를
아레나 0에서 할당한다 [S: `func_020ea48c`, autoload_2, `port/shim/boot/heapinit.c` header]. 그 블록 안에
...
인터프리터의 훅이 먼저 설치된다 [S: `port/interp/interp_boot.c:28-30, 404-418`]. 네이티브
초기 기동, 네이티브 `__init_cpp`, 네이티브 사운드 부팅은 모두 건너뛰며, 포트는
stdout에 그렇게 밝힌다 [S: `port/platform/win32.c:2355-2405`].

부팅 줄은 레지스트리 크기, 진입점, 스택을
`acww interp: BOOT via interpreter, registry entries=N entry=0x02000c39 stack=0x027e3e00` 형태로 알린다
[S: `port/interp/interp_boot.c:395-401`]. 인터프리트되는 시스템 스택 상단은 `0x027e3e00`이며,
...
`off-D16`]. 그리고 사운드 커맨드 계층은 인터프리트된 초기 기동 이후로 옮겨야 했는데,
...
포트는 주소로 이를 공급한다 [S: `port/tools/interp_registry.py:77-80`;
E: `docs/log/cycle40-keyboard-gate-probe.md` ENTRY40, `off-D18`, where the read spun forever].

### 부팅 이후: 프레임이란 무엇인가

NDS에는 프레임 루프가 없다 [S: `port/shim/os/vblank.c:11-14`]. 게임의 루프는 `VBlankIntrWait`
(`0x020002c7`, BIOS `IntrWait`를 둘러싼 세 명령어)에서 블록하며 끝나고, 수직 블랭크
인터럽트가 시간을 흐르게 하는 것이다 [S: `VBlankIntrWait`, main, `port/shim/os/vblank.c` header].
유휴 스레드의 본체 전체는 `while (1) OS_Halt();`이며, `OS_Halt`는 CP15의
인터럽트 대기(wait-for-interrupt)이다 [S: `OSi_IdleThreadProc` / `OS_Halt`, itcm, `port/shim/os/halt.c` header].
...
되감길 때 프레임 경계를 호출한다 [S: `port/interp/interp_boot.c:239-254`;
E: `docs/log/cycle40-keyboard-gate-probe.md` CARD40..SND40, `off-D36`]. 이 규칙이 없으면
상태 머신 경로의 어떤 것도 프레임 경계에 전혀 도달하지 못했다
[E: same run]. `threads-and-interrupts.md`를 참고하라.
...
탭은 그렇지 않다 [E+O: same comparison]. **이 분기는 해결되었다(ORACLE42)**: 24,600의 탭은
...
|---|---|---|---|
| `Entry` (`0x02000800`) | main | crt0: CP15, 스택, OAM/팔레트 클리어, 캐시 플러시 | [S: `port/platform/nitromain.c` header] |
| `func_020b1a74` (Thumb) | main | 하드웨어, 무선, 파일시스템 초기 기동 | [S: `port/interp/interp_boot.c:29`] |
| `func_020ea48c` | autoload_2 | 초기 기동의 마지막 호출: 아레나 0에서 게임 힙을 분할 | [S: `port/shim/boot/heapinit.c` header] |
| `func_02138968` (ARM) | main | `__init_cpp`, C++ 정적 초기화자 | [S: `port/interp/interp_boot.c:30`] |
| `0x02000c38` (Thumb, 0x60 bytes) | main | 진짜 `NitroMain`; 여기서 시작하는 심볼 없음 | [S: `port/platform/nitromain.c` header] |
| `OS_InitTick` / `OS_InitAlarm` / `OS_InitThread` | autoload_2 | OS 서비스, 이 순서대로 | [S: `port/platform/nitromain.c:33-35`] |
| `OS_SetUserExceptionHandler` (`0x0211628c`) | autoload_2 | `func_0206e6c4`를 인수 `0x0220433c`와 함께 설치 | [S: `port/platform/nitromain.c:36, 46`] |
| `OS_EnableInterrupts` (`0x01ffa314`) | itcm | 인터럽트 켜기 | [S: `port/platform/nitromain.c:37`] |
| `func_0206e518`, `func_020b1b84`, `func_0206e560` | main | 게임으로의 인계; 절대 반환하지 않음 | [S: `port/platform/nitromain.c:75, 84-85`] |
| `func_0206e6c4` / `func_0206e3f4` | main | 치명적 경로: 레지스터 테이블 저장 후 크래시 화면 | [E: cycle40 log, CARD40..SND40] |
| `VBlankIntrWait` (`0x020002c7`) | main | 게임이 블록하는 프레임 경계 | [S: `port/shim/os/vblank.c` header] |
| `OS_Halt` | itcm | 유휴 스레드의 본체 | [S: `port/shim/os/halt.c` header] |
| `func_0200149c` | main | 프레임 동기화; `VCOUNT`에서 스핀 | [E: cycle40 log, CARD40..SND40 `off-D36`] |

## 읽고 쓰는 데이터

| 주소 | 의미 | 쓰는 쪽 | 읽는 쪽 |
|---|---|---|---|
| `0x04000208` (`REG_IME`) | 인터럽트 마스터 활성화; 플래그 경로에서 두 번 1로 설정됨 | `NitroMain` | 인터럽트 컨트롤러 [S: `port/platform/nitromain.c:49, 68-81`] |
| `0x027ffc20` | 부팅 플래그; 1이면 두 번째 부팅 경로 선택 | `NitroMain` 이전에 설정됨 | `NitroMain` [S: `port/platform/nitromain.c:50, 77`] |
| `0x0220433c` | 사용자 예외 핸들러의 인수 워드 | ROM 리터럴 풀 | `OS_SetUserExceptionHandler` [S: `port/platform/nitromain.c:48`] |
| `0x0213fde0` | 치명적 폴트 시의 예외 레지스터 테이블 | `func_0206e6c4` | 크래시 화면, 그리고 `ACWW_INTERP_PEEK` [E: cycle40 log, CARD40..SND40] |
| `0x027fff9c` | crt0가 쓰는 인터럽트 벡터 슬롯 | `Entry` | 벡터 [S: `port/platform/win32.c:78-83`] |
| `0x027e3e00` | 인터프리트되는 시스템 스택 상단(포트 전용) | `interp_boot.c` | 인터프리터 [S: `port/interp/interp_boot.c:31`] |
...
오라클이 같은 순간을 고정할 수 있게 한다 [S: `docs/kb/hybrid/recipes.md` sections 3 and 6].
